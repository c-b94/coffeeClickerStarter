...
function getUnlockedProducers(data) {
  ...
  const prod = data.producers;
  function isItUnlocked(value) {
      if (value.unlocked === true) {
        return true;
      }
      return false;
  }
  return prod.filter(isItUnlocked);
}
...
function passthroughOnEvent(event){
  buyButtonClick(event, data)
}
producerContainer.addEventListener('click',passthroughOnEvent);
